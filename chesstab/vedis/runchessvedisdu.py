...
but use of an vedis feature).  DPT deferred update requires that it's
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            chessvedisdu = importlib.import_module(
                os.path.basename(packageroot) + ".vedis.chessvedisdu"
            )
            ...
            if " '__file__' " not in str(msg):
                raise
            from . import chessvedisdu
            from ..gui import chessdu

        cdu = chessdu.ChessDeferredUpdate(
            deferred_update_method=chessvedisdu.chess_vedisdu,
            database_class=chessvedisdu.ChessDatabase,
        )
    except Exception:
        try:
