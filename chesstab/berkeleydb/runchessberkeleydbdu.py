...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            chessberkeleydbdu = importlib.import_module(
                os.path.basename(packageroot) + ".berkeleydb.chessberkeleydbdu"
            )
            ...
            if " '__file__' " not in str(msg):
                raise
            from . import chessberkeleydbdu
            from ..gui import chessdu

        cdu = chessdu.ChessDeferredUpdate(
            deferred_update_method=chessberkeleydbdu.chess_dbdu,
            database_class=chessberkeleydbdu.ChessDatabase,
        )
    except Exception:
        try:
