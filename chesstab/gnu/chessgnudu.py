...
class ChessDatabase(gnudu_database.Database):
        ...
        if not kargs.get("allowcreate", False):
            try:
                ...
                if __name__ == "__main__":
                    ...
                    raise ChessgnuduError("dbm.gnu description invalid")

        try:
            super(ChessDatabase, self).__init__(names, gnufile, **kargs)
        except ChessgnuduError:
            if __name__ == "__main__":
                raise
            else:
                raise ChessgnuduError("dbm.gnu description invalid")
    ...
    def open_context_prepare_import(self):
        ...
        No preparation actions thet need database open for dbm.gnu.

        """
        return True

    def get_archive_names(self, files=()):
        """Return dbm.gnu database file and existing operating system files."""
