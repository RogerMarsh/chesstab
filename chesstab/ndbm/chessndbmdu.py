...
class ChessDatabase(ndbmdu_database.Database):
        ...
        if not kargs.get("allowcreate", False):
            try:
                ...
                if __name__ == "__main__":
                    ...
                    raise ChessndbmduError("dbm.ndbm description invalid")

        try:
            super(ChessDatabase, self).__init__(names, ndbmfile, **kargs)
        except ChessndbmduError:
            if __name__ == "__main__":
                raise
            else:
                raise ChessndbmduError("dbm.ndbm description invalid")
    ...
    def open_context_prepare_import(self):
        ...
        No preparation actions thet need database open for dbm.ndbm.
