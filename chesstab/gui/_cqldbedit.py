...
class CQLDbEditBase(EditText, DataEdit):
    ...
    def edit(self, commit=True):
        ...
        self.prepare_cql_statement()
        try:
            cqls.update_cql_statement_games(self.newobject, commit=False)
        except cqlstatement.CQLStatementError as exc:
            if commit:
                self.datasource.dbhome.backout()
            newview = self.newview
            widget = newview.score
            tag_ranges = widget.tag_ranges(newview.TEXT_DATA)
            evaluator = self.newobject.value.query_container.evaluator
            if evaluator.error_location is not None:
                start = widget.index(evaluator.error_location)
                if not tag_ranges or len(tag_ranges) > 2:
                    widget.tag_add(newview.ERROR_TAG, start)
                else:
                    widget.tag_add(
                        newview.ERROR_TAG,
                        start,
