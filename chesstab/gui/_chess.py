...
class Chess(Bindings):
    ...
    def _create_menu3_game(self, menus, menubar):
        """Do nothing.  Menu entries for a new game are not created."""

    def _create_menu4_position(self, menus, menubar):
        """Do nothing.  Menu entries for a position query are not created."""

    def _add_cql_scan_to_menu(self, menu10101):
        """Do nothing.  Menu entries to export PGN file are not created."""
    ...
    def _help_guide(self):
        """Do nothing.  Override if this method provides help."""
    ...
    def _create_chessui_instance(self, toppane, menu7, menu8, toolbarframe):
        """Return None.  Override in subclass to create User Interface."""
        raise ChessError("Override '_create_chessui_instance()' in a subclass")
    ...
    def _create_cql_display_insert_instance(self):
        """Subclass must override and return a CQLInsert instance."""
        raise ChessError(
            "".join(
                (
                    "Use a subclass which overrides this method and ",
                    "returns a CQLInsert instance",
                )
            )
        )
