...
class ChessDatabase(unqlitedu_database.Database):
        ...
        if not kargs.get("allowcreate", False):
            try:
                ...
                if __name__ == "__main__":
                    ...
                    raise ChessunqliteduError("unqlite description invalid")

        try:
            super(ChessDatabase, self).__init__(names, unqlitefile, **kargs)
        except ChessunqliteduError:
            if __name__ == "__main__":
                raise
            else:
                raise ChessunqliteduError("unqlite description invalid")
    ...
    def open_context_prepare_import(self):
        ...
        No preparation actions thet need database open for unqlite.

        """
        return True

    def get_archive_names(self, files=()):
        """Return unqlite database file and existing operating system files."""
