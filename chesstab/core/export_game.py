...
# This pattern is needed to create pgn-like files accepted by CQL program,
# and preserve the format generated by Database - Export - Games - Text
# menu action, when " " not "" is used to join tokens for games stored on
# database.  At July 2025 CQL crashes when presented with files where
# tokens are separated by a single space and games by a single '\n' but
# this looks accidental rather than intentional.
# Both methods which generate files with this pattern take nearly 2.5
# times longer to do so than by just outputting space delimited tokens as
# found in database record.
# Note that starting from null delimited tokens takes nearly 20 times
# longer because full parsing of the games, with the equivalent pattern
# from pgn_read, is required.
_TEXT_FORMAT_TOKENS = r"|".join(
    (
        r"".join(
            (
                r"(?#Tag Pair)(\[\s*[A-Za-z0-9_]+\s*",
                r'"(?:[^\\"\t\r\f\v\n]*)',
                r'(?:\\[^\t\r\f\v\n][^\\"\t\r\f\v\n]*)*)"\s*\]',
            )
        ),
        r"(?#Move characters)([KQRBNa-h1-8x=/0O-]+)",
        ...
        pgn_read.core.constants.ESCAPED,
        pgn_read.core.constants.PASS,
        pgn_read.core.constants.CHECK,
        pgn_read.core.constants.TRADITIONAL,
    )
)
text_format_tokens = re.compile(_TEXT_FORMAT_TOKENS)
            ...
            with open(filename, "w", encoding=_ENCODING) as gamesout:
                ...
                while current_record:
                    ...
                    for token in text_format_tokens.finditer(
                        ...
                        if (
                            groups[3] is not None  # EOL Comment.
